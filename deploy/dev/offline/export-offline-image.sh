...
#
# Usage:
#   ./deploy/dev/offline/export-offline-image.sh                 # CPU (asr-webservice:offline)
#   ./deploy/dev/offline/export-offline-image.sh gpu             # GPU (asr-webservice:offline-gpu)
#   ./deploy/dev/offline/export-offline-image.sh <image[:tag]>   # explicit image reference
set -euo pipefail
...
if ! docker image inspect "$image" >/dev/null 2>&1; then
  echo "ERROR: image '${image}' not found - run ./deploy/dev/offline/build-offline-image.sh first." >&2
  exit 1
fi
