...
# run ./deploy/dev/docker-rebuild.sh first to build/refresh them), waits for it to
# become ready, then runs the sample client (docker-compose.yml "client" service) against it.
#
# Usage:
#   ./deploy/dev/docker-start.sh            # CPU (docker-compose.yml)
#   ./deploy/dev/docker-start.sh gpu        # GPU (docker-compose.gpu.yml)
#
