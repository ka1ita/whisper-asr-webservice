...
Docker builds: `Dockerfile` (CPU) and `Dockerfile.gpu` (CUDA); see `docker-compose.yml` / `docker-compose.gpu.yml` for local container runs. Both compose files load `.env` (copy from `.env.example`; git-ignored) via `env_file` for secrets like `HF_TOKEN`, and define a `client` service (profile `client`, so it's excluded from a plain `docker compose up`) that runs the sample client in `client/` against the ASR service. `scripts/` is bash-only (no PowerShell): `./scripts/docker-rebuild.sh` rebuilds the ASR service image (always) and the client image (only if `client/` changed, hash-gated via `client/.docker-build-hash`); `./scripts/docker-start.sh` starts the ASR service, waits for readiness, and runs the client against it (no rebuild — run `docker-rebuild.sh` first if images don't exist yet); `./scripts/docker-stop.sh` stops the service via `docker compose down`. For fast local iteration without Docker, `./scripts/python-start.sh` runs the client directly with Python (manages a venv at `client/.venv`, reinstalling `requirements.txt` only when it changes). All accept an optional `gpu` arg to target `docker-compose.gpu.yml`.
