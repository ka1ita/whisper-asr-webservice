...
Docker builds: `Dockerfile` (CPU) and `Dockerfile.gpu` (CUDA); see `docker-compose.yml` / `docker-compose.gpu.yml` for local container runs. Both compose files load `.env` (copy from `.env.example`; git-ignored) via `env_file` for secrets like `HF_TOKEN`, and define a `client` service (profile `client`, so it's excluded from a plain `docker compose up`) that runs the sample client in `client/` against the ASR service. `deploy/dev/` is bash-only (no PowerShell): `./deploy/dev/docker-rebuild.sh` rebuilds the ASR service image (always) and the client image (only if `client/` changed, hash-gated via `client/.docker-build-hash`); `./deploy/dev/docker-start.sh` starts the ASR service, waits for readiness, and runs the client against it (no rebuild — run `docker-rebuild.sh` first if images don't exist yet); `./deploy/dev/docker-stop.sh` stops the service via `docker compose down`. For fast local iteration without Docker, `./deploy/dev/python-start.sh` runs the client directly with Python (manages a venv at `client/.venv`, reinstalling `requirements.txt` only when it changes). All accept an optional `gpu` arg to target `docker-compose.gpu.yml`. Offline/air-gapped deployment tooling lives in `deploy/dev/offline/`; exported image tarballs land in `deploy/prod/dist/`. `deploy/prod/` is the self-contained runtime for the isolated server: `docker-compose.yml` running the preloaded `asr-webservice:offline` image (config — `HF_TOKEN`, `ASR_ENGINE`, `ASR_MODEL`, GPU tag via `ASR_IMAGE` — comes from a `.env` next to it, no file edits) plus `service-{start,stop,restart,logs}.sh` wrappers; the scripts `cd` to their own directory, so the folder can be copied to the server wholesale alongside the image tar.
...
- `app/asr_models/{openai_whisper,faster_whisper,mbain_whisperx,gigaam}_engine.py` — the four interchangeable engine implementations (`openai_whisper`, `faster_whisper`, `whisperx`, `gigaam`). Adding a new engine means implementing `ASRModel` and registering it in the factory. `gigaam` (Russian-focused, from [GigaAM](https://github.com/salute-developers/GigaAM)) takes a file path rather than an in-memory array, so its engine writes the decoded audio to a temp WAV first; it has no native diarization, so it reuses the `whisperx`-based pipeline in `app/diarization.py`.
- `app/diarization.py` — shared pyannote-based diarization pipeline (`load_diarization_pipeline`, gated on `CONFIG.HF_TOKEN`) and the torch `weights_only` unpickling workaround, used by both the `whisperx` and `gigaam` engines.
