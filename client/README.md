...
Or skip the manual venv/install steps with `./deploy/dev/python-start.sh` (run from the repo
...
```shell
./deploy/dev/python-start.sh
./deploy/dev/python-start.sh --config other.yaml   # extra args are forwarded
```
...
./deploy/dev/docker-rebuild.sh      # build/refresh images (first run, or after changes)
./deploy/dev/docker-start.sh        # CPU
./deploy/dev/docker-start.sh gpu    # GPU
```
...
```shell
./deploy/dev/docker-rebuild.sh        # CPU compose project
./deploy/dev/docker-rebuild.sh gpu    # GPU compose project
```
...
```shell
./deploy/dev/docker-stop.sh        # CPU
./deploy/dev/docker-stop.sh gpu    # GPU
```
